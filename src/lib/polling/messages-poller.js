  ...
  switch (nativeResource.messagetype) {
    case 'RichText/UriObject':
      ...
      return formatUriObjectResource(
        formatFileResource(formatGenericMessageResource(nativeResource, nativeResource.messagetype), nativeResource),
        nativeResource,
      );
    ...
    default:
      // tslint:disable-next-line:max-line-length
      throw new Error(
        `Unknown ressource.messageType (${JSON.stringify(nativeResource.messagetype)}) for resource:\n${JSON.stringify(
          nativeResource,
          null,
          '\t',
        )}`,
...
function formatEventMessage(native) {
  ...
  switch (native.resourceType) {
    ...
    default:
      // tslint:disable-next-line:max-line-length
      throw new Error(
        `Unknown EventMessage.resourceType (${JSON.stringify(native.resourceType)}) for Event:\n${JSON.stringify(
          native,
        )}`,
      );
  }
